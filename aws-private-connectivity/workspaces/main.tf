...
resource "databricks_grants" "sudoers" {
  catalog = data.databricks_catalog.main.name

  grant {
    principal  = data.databricks_group.sudoers.display_name
    privileges = ["ALL_PRIVILEGES"]
  }

  depends_on = [data.databricks_catalog.main]
}
