locals {
  ...
  dmz_cidr_block         = cidrsubnet(local.cidr_block, 2, 1)
  public_cidr_block      = cidrsubnet(local.cidr_block, 2, 2)
  databricks_cidr_block  = cidrsubnet(local.cidr_block, 2, 3)
}
