...
def get_query() -> Query:
    ...
    return {**query, "paths": json.loads(query["paths"])}


def get_current_allowlist_matchers(client: WorkspaceClient) -> list[ArtifactMatcher]:
    result: ArtifactAllowlistInfo = client.artifact_allowlists.get(artifact_type=ArtifactType.INIT_SCRIPT)
...
def union(prefix: str, current: list[ArtifactMatcher], paths: list[str]) -> dict[str, Matcher]:
    ...
    for path in paths:
        [_prefix, catalog, schema, volume, *rest] = path.split("/")
    ...
    for matcher in current:
        ...
        [_prefix, catalog, schema, volume, *rest] = matcher.artifact.split("/")
...
def main() -> Result:
    ...
    current: list[ArtifactMatcher] = get_current_allowlist_matchers(client=client)
